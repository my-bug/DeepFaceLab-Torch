...
def save_faceset_metadata_folder(input_path):
    ...
    io.log_info (f"正在保存元数据到 {str(metadata_filepath)}\r\n")
    ...
    io.log_info("现在你可以编辑这些图片了。")
    io.log_info("!!! 请保持目录内文件名不变。")
    io.log_info("你可以调整图片尺寸；恢复元数据时会自动缩放回原始尺寸。")
    io.log_info("编辑完成后，请运行“恢复 faceset 元数据”。")

def restore_faceset_metadata_folder(input_path):
    input_path = Path(input_path)

    metadata_filepath = input_path / 'meta.dat'
    io.log_info (f"正在从 {str(metadata_filepath)} 恢复元数据。\r\n")

    if not metadata_filepath.exists():
        io.log_err(f"未找到 {str(metadata_filepath)}。")
    ...
    for filepath in io.progress_bar_generator(pathex.get_image_paths(input_path, image_extensions=['.jpg'], return_Path_class=True), "处理中"):
        ...
        if saved_data is None:
            io.log_info(f"{filepath} 没有已保存的元数据")
            continue
