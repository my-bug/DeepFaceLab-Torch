...
class SampleGeneratorFace(SampleGeneratorBase):
    def __init__ (self, samples_path, debug=False, batch_size=1,
        ...
        rnd_seed = kwargs.get('rnd_seed', None)
        if rnd_seed is None:
            seed_env = os.environ.get('DFL_INDEX_SEED', None)
            if seed_env is not None and str(seed_env).strip() != '':
                try:
                    rnd_seed = int(seed_env)
                except Exception:
                    rnd_seed = None
        ...
        if uniform_yaw_distribution:
            ...
            # Index2DHost 当前不支持 rnd_seed；该路径仍保持原行为。
    ...
    def close(self):
        gens = getattr(self, 'generators', None)
        if not gens:
            return
        for g in gens:
            try:
