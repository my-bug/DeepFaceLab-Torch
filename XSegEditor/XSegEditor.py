...
class CanvasConfig():
    ...
    def __init__(self,
        ...
        if color_schemes is None:
            def _get_cursor(attr_name):
                c = getattr(QCursorDB, attr_name, None)
                if c is not None:
                    return c
                # Fallback cursor (created after QApplication exists)
                return QCursor(Qt.CrossCursor)

            color_schemes = [
                    ColorScheme( QColor(192,0,0,0), QColor(192,0,0,72), QColor(192,0,0), 2, QColor(255,255,255), _get_cursor('cross_red') ),
                    ColorScheme( QColor(0,192,0,0), QColor(0,192,0,72), QColor(0,192,0), 2, QColor(255,255,255), _get_cursor('cross_green') ),
                    ColorScheme( QColor(0,0,192,0), QColor(0,0,192,72), QColor(0,0,192), 2, QColor(255,255,255), _get_cursor('cross_blue') ),
                    ]
...
class QCanvasOperator(QWidget):
    ...
    def img_to_cli_rect(self, rect):
        ...
        return QRect ( *xy.astype(np.int32), *xy2.astype(np.int32) )
    ...
    # ====================================================================================
    @staticmethod
    def _qt_mods_to_int(mods):
        """Return int modifiers across PyQt5/PySide6.

        PyQt5 often allows int(Qt.KeyboardModifiers) directly.
        PySide6/Qt6 returns a flags object where the numeric value is in `.value`.
        """
        try:
            return int(mods)
        except Exception:
        ...
        # Last-resort: treat as no modifiers.
        return 0

    @staticmethod
    def _qt_event_pos_to_qpoint(ev):
        """Return widget-local position as QPoint across PyQt5/PySide6.

        Qt5: QMouseEvent/QWheelEvent exposes .pos() -> QPoint
        Qt6: exposes .position() -> QPointF
        """
    ...
    def on_keyPressEvent(self, ev):
        if not self.initialized:
            return
        key = ev.key()
        key_mods = self._qt_mods_to_int(ev.modifiers())
        if self.op_mode == OpMode.DRAW_PTS:
    ...
    def mousePressEvent(self, ev):
        ...
        self.update_mouse_info(QPoint_to_np(self._qt_event_pos_to_qpoint(ev)))
    ...
    def wheelEvent(self, ev):
        ...
        mods = self._qt_mods_to_int(ev.modifiers())
        delta = ev.angleDelta()

        cli_pt = QPoint_to_np(self._qt_event_pos_to_qpoint(ev))
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            # PyQt5 exposes QPainter.HighQualityAntialiasing; PySide6 (Qt6) does not.
            if hasattr(QPainter, 'Antialiasing'):
                _try_set_hint(QPainter.Antialiasing)
            elif hasattr(QPainter, 'RenderHint') and hasattr(QPainter.RenderHint, 'Antialiasing'):
                _try_set_hint(QPainter.RenderHint.Antialiasing)

            if hasattr(QPainter, 'HighQualityAntialiasing'):
                _try_set_hint(QPainter.HighQualityAntialiasing)
            elif hasattr(QPainter, 'RenderHint') and hasattr(QPainter.RenderHint, 'HighQualityAntialiasing'):
                _try_set_hint(QPainter.RenderHint.HighQualityAntialiasing)

            if hasattr(QPainter, 'SmoothPixmapTransform'):
                _try_set_hint(QPainter.SmoothPixmapTransform)
            elif hasattr(QPainter, 'RenderHint') and hasattr(QPainter.RenderHint, 'SmoothPixmapTransform'):
                _try_set_hint(QPainter.RenderHint.SmoothPixmapTransform)
...
def start(input_dirpath):
    ...
    if PackedFaceset.path_contains(input_dirpath):
        io.log_info (f'\n{input_dirpath} 包含已打包的 faceset！请先解包。\n')
        return 1
