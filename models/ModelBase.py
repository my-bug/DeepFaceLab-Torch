...
class ModelBase(object):
    ...
    def ask_autobackup_hour(self, default_value=0):
        ...
        self.options['autobackup_hour'] = io.input_int(f"每 N 小时自动备份一次", default_autobackup_hour, add_info="0..24", help_message="每 N 小时自动备份模型文件（含预览图）。最新备份位于 model/<>_autobackups/01")
    ...
    def get_summary_text(self):
        ...
        summary_text = []
        summary_text += [f'=={" 模型摘要 ":=^{width_total}}=='] # Model/status summary
        summary_text += [f'=={" "*width_total}==']
        summary_text += [f'=={"模型名称": >{width_name}}: {self.get_model_name(): <{width_value}}=='] # Name
        summary_text += [f'=={" "*width_total}==']
        summary_text += [f'=={"当前迭代": >{width_name}}: {str(self.get_iter()): <{width_value}}=='] # Iter
        summary_text += [f'=={" "*width_total}==']
        ...
        if len(self.device_config.devices) == 0:
            summary_text += [f'=={"使用设备": >{width_name}}: {"CPU": <{width_value}}=='] # cpu_only
        else:
            for device in self.device_config.devices:
                summary_text += [f'=={"设备索引": >{width_name}}: {device.index: <{width_value}}=='] # GPU hardware device index
                summary_text += [f'=={"名称": >{width_name}}: {device.name: <{width_value}}=='] # GPU name
                vram_str = f'{device.total_mem_gb:.2f}GB' # GPU VRAM - Formated as #.## (or ##.##)
                summary_text += [f'=={"显存": >{width_name}}: {vram_str: <{width_value}}==']
        summary_text += [f'=={" "*width_total}==']
        summary_text += [f'=={"="*width_total}==']
        summary_text = "\n".join (summary_text)
        return summary_text
