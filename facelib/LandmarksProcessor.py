...
def get_canonical_68(output_size: int = 256) -> np.ndarray:
    """Canonical 68pt template in aligned face space (pixel coords).

    Points 17..67 are taken from the built-in 2D template.
    Jawline 0..16 is synthesized to make masks and alignment stable.
    """
    pts = np.zeros((68, 2), dtype=np.float32)

    # Jawline in normalized [0..1] -> scaled to output_size
    cx, cy = 0.5, 0.82
    rx, ry = 0.48, 0.32
    for i in range(17):
        ...
        pts[i] = (np.clip(x, 0.0, 1.0), np.clip(y, 0.0, 1.0))

    for j, p in enumerate(landmarks_2D):
        idx = 17 + j
        if idx >= 68:
            break
        pts[idx] = (float(p[0]), float(p[1]))

    return (pts * float(output_size)).astype(np.float32)

def get_canonical_5_from_68(pts68: np.ndarray) -> np.ndarray:
    left_eye = np.mean(pts68[36:42], axis=0)
    right_eye = np.mean(pts68[42:48], axis=0)
    nose_tip = pts68[30]
    mouth_l = pts68[48]
    mouth_r = pts68[54]
    return np.stack([left_eye, right_eye, nose_tip, mouth_l, mouth_r]).astype(np.float32)

def get_transform_mat_from_5(kps5: np.ndarray, output_size: int, face_type, scale: float = 1.0):
    """DFL-style transform matrix from detector 5 points.

    We first estimate an image->unit-square similarity transform, then reuse
    the same padding/centering logic as get_transform_mat.
    """
    kps5 = np.asarray(kps5, dtype=np.float32)
    if kps5.shape != (5, 2):
        raise Exception('kps5 must be (5,2)')

    templ68 = get_canonical_68(1)  # unit template
    templ5 = get_canonical_5_from_68(templ68)

    # Ensure canonical order for kps5
    dst5 = kps5.copy()
    if dst5[0, 0] > dst5[1, 0]:
        dst5[[0, 1]] = dst5[[1, 0]]
    if dst5[3, 0] > dst5[4, 0]:
        dst5[[3, 4]] = dst5[[4, 3]]

    # Similarity transform image->unit (use umeyama for broad OpenCV compatibility)
    mat = umeyama(dst5, templ5, True)[0:2].astype(np.float32)
    ...
    if face_type == FaceType.WHOLE_FACE:
        ...
        yaw = estimate_averaged_yaw(transform_points(kps5, mat, False))
    ...
    l_t = np.asarray(l_t, dtype=np.float32)
...
def get_transform_mat (image_landmarks, output_size, face_type, scale=1.0):
    ...
    # cv2.getAffineTransform requires float32 arrays of shape (3,2)
    l_t = np.asarray(l_t, dtype=np.float32)
    pts2 = np.float32(((0, 0), (output_size, 0), (output_size, output_size)))
