...
class MergerConfig(object):
    ...
    def __init__(self, type=0,
        ...
        self.sharpen_dict = {0:"无", 1:'方框', 2:'高斯'}
    ...
    def ask_settings(self):
        ...
        io.log_info(s)
        self.sharpen_mode = io.input_int ("", 0, valid_list=self.sharpen_dict.keys(), help_message="通过锐化滤镜增强细节。")

        if self.sharpen_mode != 0:
            self.blursharpen_amount = np.clip ( io.input_int ("选择 模糊/锐化 强度", 0, add_info="-100..100"), -100, 100 )
    ...
    def to_string(self, filename):
        ...
        r += f"模糊/锐化强度 : {self.blursharpen_amount}\n"
        return r
...
class MergerConfigMasked(MergerConfig):
    ...
    def ask_settings(self):
        ...
        if 'raw' not in self.mode:
            if self.mode == 'hist-match':
                self.masked_hist_match = io.input_bool("是否对 mask 区域做直方图匹配？", True)

            if self.mode == 'hist-match' or self.mode == 'seamless-hist-match':
                self.hist_match_threshold = np.clip ( io.input_int("直方图匹配阈值", 255, add_info="0..255"), 0, 255)
        ...
        if 'raw' not in self.mode:
            self.erode_mask_modifier = np.clip ( io.input_int ("选择 mask 腐蚀修正值", 0, add_info="-400..400"), -400, 400)
            self.blur_mask_modifier =  np.clip ( io.input_int ("选择 mask 模糊修正值", 0, add_info="0..400"), 0, 400)
            self.motion_blur_power = np.clip ( io.input_int ("选择 运动模糊 强度", 0, add_info="0..100"), 0, 100)

        self.output_face_scale = np.clip (io.input_int ("选择 输出人脸缩放修正值", 0, add_info="-50..50" ), -50, 50)
        ...
        if 'raw' not in self.mode:
            ...
            self.bicubic_degrade_power = np.clip ( io.input_int ("选择 双三次缩放降质强度", 0, add_info="0..100"), 0, 100)
            self.color_degrade_power = np.clip (  io.input_int ("最终图像颜色降质强度", 0, add_info="0..100"), 0, 100)
...
class MergerConfigFaceAvatar(MergerConfig):
    ...
    #override
    def ask_settings(self):
        self.add_source_image = io.input_bool("是否添加源图用于对比？", False, help_message="在结果中附加源图以便对比。")
        super().ask_settings()
