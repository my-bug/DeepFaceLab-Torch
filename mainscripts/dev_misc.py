...
def extract_vggface2_dataset(input_dir, device_args={} ):
    ...
    if not input_path.exists():
        raise ValueError('未找到输入目录，请确认该路径存在。')

    bb_csv = input_path / 'loose_bb_train.csv'
    if not bb_csv.exists():
        raise ValueError('未找到 loose_bb_train.csv，请确认该文件存在。')
...
"""
    ...
    while i < data_len-1:
        ...
        io.log_info (f"正在处理：{str(cur_output_path)}，{i}/{data_len} ")
    ...
    for dir_name in dir_names:
        ...
        io.log_info (f"正在处理：{str(cur_input_path)} ")
        ...
        io.log_info (f"正在排序：{str(cur_output_path)} ")
...
def extract_vggface2_dataset(input_dir, device_args={} ):
    ...
    for dir_name in dir_names:
        ...
        io.log_info (f"正在排序：{str(cur_input_path)} ")
        Sorter.main (input_path=str(cur_output_path), sort_by_method='hist')
...
#（终端用户工作流未使用）
def dev_gen_mask_files(input_dir):
    """从 DFL 图像中内嵌的 mask 导出单独的 *_mask 文件。

    这是一个开发/辅助入口，由 `main.py dev_test` 与 `run.py` 调用。
    """

    from . import Util

    Util.export_faceset_mask(input_dir)


def dev_test_68(input_dir ):
    # 处理带 .pts 文件的 68 关键点数据集
    input_path = Path(input_dir)
    if not input_path.exists():
        raise ValueError('未找到 input_dir。请确认路径存在。')
...
def extract_umd_csv(input_file_csv,
    ...
    if not input_file_csv_path.exists():
        raise ValueError('未找到 input_file_csv。请确认路径存在。')
    ...
    for i in range(1, len(strings)):
        ...
        if keys_len != len(values):
            io.log_err("CSV 文件中存在格式错误的行，已跳过。")
            continue
    ...
    faces_detected = 0
    if len(data) > 0:
        io.log_info ("正在执行第 2 次处理（从 CSV 读取）...")
...
def dev_resave_pngs(input_dir):
    input_path = Path(input_dir)
    if not input_path.exists():
        raise ValueError('未找到 input_dir，请确认该路径存在。')
...
def dev_test(input_dir):
    ...
    for filepath in io.progress_bar_generator(pathex.get_paths(input_path), "处理中"):
        if filepath.suffix == '.txt':
            
            image_filepath = filepath.parent / f'{filepath.name.split("_")[0]}.png'
            if not image_filepath.exists():
                print(f'{image_filepath} 不存在，已跳过') 
    ...
    if len(data) > 0:
        ...
        for filename in io.progress_bar_generator(pathex.get_image_paths (output_path), "处理中"):
            filepath = Path(filename)
