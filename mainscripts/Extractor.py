...
class ExtractSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if 'rects' in self.type or self.type == 'all':
                data = ExtractSubprocessor.Cli.rects_stage(
                    data=data,
                    image=image,
            ...
            if 'landmarks' in self.type or self.type == 'all':
                data = ExtractSubprocessor.Cli.landmarks_stage(
                    data=data,
                    image=image,
            ...
            if min(h, w) < 128:
                ...
                for rot in ([0, 90, 270, 180]):
                    ...
                    if len(rects) != 0:
                        ...
                        break
                if max_faces_from_image is not None and max_faces_from_image > 0 and len(data.rects) > 0:
                    data.rects = data.rects[0:max_faces_from_image]
            return data

        @staticmethod
        def landmarks_stage(data, image, landmarks_extractor, rects_extractor):
            h, w, ch = image.shape
        ...
        @staticmethod
        def final_stage(data, image, face_type, image_size, jpeg_quality, output_debug_path=None, final_output_path=None):
            ...
            for rect, image_landmarks in zip(rects, landmarks):
                ...
                if face_type == FaceType.MARK_ONLY:
                    ...
                    if not data.manual and face_type <= FaceType.FULL_NO_ALIGN and landmarks_area > 4 * rect_area:
                        continue
            ...
            if output_debug_path is not None:
                output_debug_filepath = output_debug_path / (filepath.stem + '.jpg')
                cv2_imwrite(output_debug_filepath, debug_image, [int(cv2.IMWRITE_JPEG_QUALITY), 50])
    ...
    def get_devices_for_config(type, device_config):
        ...
        if 'rects' in type or 'landmarks' in type or 'all' in type:
            if not cpu_only:
                ...
                for device in devices:
                    result.append((device.index, 'GPU', device.name, device.total_mem_gb))
                return result
            else:
                if type == 'landmarks-manual':
                    return [(0, 'CPU', 'CPU', 0)]
                return [(i, 'CPU', f'CPU{i}', 0) for i in range(min(8, multiprocessing.cpu_count() // 2))]

        if type == 'final':
            cpu_count = min(8, multiprocessing.cpu_count())
            return [(i, 'CPU', f'CPU{i}', 0) for i in (range(cpu_count) if not DEBUG else [0])]
    ...
    def __init__(
        ...
        self.devices = ExtractSubprocessor.get_devices_for_config(self.type, self.device_config)

        super().__init__('Extractor', ExtractSubprocessor.Cli, 999999 if type == 'landmarks-manual' or DEBUG else 120)
            ...
            while len(self.input_data) > 0:
                ...
                if len(data_rects) == 0:
                    ...
                    while True:
                        ...
                        elif self.x != new_x or self.y != new_y or self.rect_size != new_rect_size or self.extract_needed:
                            self.x = new_x
                            self.y = new_y
...
class DeletedFilesSearcherSubprocessor(Subprocessor):
    def __init__(self, input_paths, debug_files):
        self.input_paths = list(input_paths)
        self.deleted_filenames = set([Path(x).name.split('_')[0] for x in debug_files])
        self.result = []

        super().__init__('DeletedFilesSearcher', DeletedFilesSearcherSubprocessor.Cli)

    # override
    def on_clients_initialized(self):
        io.progress_bar(None, len(self.input_paths))
    ...
    # override
    def process_info_generator(self):
        base_dict = {'deleted_filenames': list(self.deleted_filenames)}
    ...
    # override
    def on_data_return(self, host_dict, data):
        self.input_paths.insert(0, data)
    ...
    class Cli(Subprocessor.Cli):
        def on_initialize(self, client_dict):
            self.deleted_filenames = set(client_dict.get('deleted_filenames', []))
            self.log_info(f"正在运行于 {client_dict.get('device_name', 'CPU')}")

        def process_data(self, filename):
            if Path(filename).name.split('_')[0] not in self.deleted_filenames:
                return filename
            return None
...
def main(
    ...
    if not manual_output_debug_fix and len(output_images_paths) > 0:
        if len(output_images_paths) > 128:
            continue_extraction = io.input_bool('是否继续提取？', True, help_message='可以从上次进度继续，但你必须再次指定与上次相同的选项。')
        ...
        if len(output_images_paths) > 128 and continue_extraction:
            try:
                ...
                io.log_err('获取上次进度索引失败，无法继续提取。')
                return
        elif input_path != output_path:
            io.input(f"\n 警告！！！\n {output_path} 目录中已存在文件！\n 这些文件将被删除。\n 按回车继续。\n")
            for filename in output_images_paths:
                Path(filename).unlink()

    device_config = nn.DeviceConfig.GPUIndexes(force_gpu_idxs or nn.ask_choose_device_idxs(choose_only_one=detector == 'manual', suggest_all_gpu=True)) if not cpu_only else nn.DeviceConfig.CPU()

    if face_type is None:
        face_type = io.input_str('人脸类型（face_type）', 'wf', ['f', 'wf', 'head'], help_message="f=full_face / wf=whole_face / head。whole_face 覆盖更完整的面部区域（含额头）；head 覆盖整个头部，但 src/dst faceset 需要 XSeg 支持。").lower()
        face_type = {'f': FaceType.FULL, 'wf': FaceType.WHOLE_FACE, 'head': FaceType.HEAD}[face_type]
    ...
    if detector is None:
        io.log_info('请选择检测器类型。')
        io.log_info('[0] S3FD')
        io.log_info('[1] 手动（manual）')
        detector = {0: 's3fd', 1: 'manual'}[io.input_int('', 0, [0, 1])]

    if output_debug is None:
        output_debug = io.input_bool(f'是否将调试图片写入 {output_debug_path.name}？', False)
    ...
    if manual_output_debug_fix:
        if not output_debug_path.exists():
            io.log_err(f'未找到 {output_debug_path}。请在“写入调试图片”开启的情况下重新提取。')
            return
        else:
            detector = 'manual'
            io.log_info('正在重新提取：仅处理在 _debug 目录中被删除的帧。')
            ...
            io.log_info('找到 %d 张图片。' % (len(input_image_paths)))
    else:
    ...
    if images_found != 0:
        ...
        if manual_fix:
            if all(np.array([d.faces_detected > 0 for d in data]) == True):
                io.log_info('所有图片均已检测到人脸，无需手动修复。')
            else:
    ...
    io.log_info('-------------------------')
    io.log_info('找到图片数：          %d' % (images_found))
    io.log_info('检测到人脸数：        %d' % (faces_detected))
    io.log_info('-------------------------')
