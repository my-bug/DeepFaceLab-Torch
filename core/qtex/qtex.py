...
def _dfl_qt_message_handler(msg_type, context, message):
    # Suppress noisy macOS Qt ICC profile parsing messages.
    # Example: "qt.gui.icc: fromIccProfile: Failed to parse description"
    try:
        if message and ('qt.gui.icc' in message or 'fromIccProfile' in message):
            return
    except Exception:
        pass
...
def install_qt_message_filter():
    """Install default Qt message filter (safe to call multiple times)."""
    global _qt_message_handler_installed
    if _qt_message_handler_installed:
        return
    try:
        qInstallMessageHandler(_dfl_qt_message_handler)
        _qt_message_handler_installed = True
    except Exception:
        # If Qt bindings are partially available, ignore.
        pass


# Install on import so all GUI entrypoints benefit.
...
def QPoint_from_np(n):
    return QPoint(*n.astype(np.int32))
