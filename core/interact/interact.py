...
def _silent_input_enabled() -> bool:
    return str(os.environ.get('DFL_SILENT_INPUT', '')).strip().lower() in (
        '1', 'y', 'yes', 'true', 'on'
    )
...
class InteractBase(object):
    ...
    def progress_bar(self, desc, total, leave=True, initial=0):
        if str(os.environ.get('DFL_DISABLE_TQDM', '')).strip().lower() in ('1', 'y', 'yes', 'true', 'on'):
            return
    ...
    def input_number(self, s, default_value, valid_list=None, show_default_value=True, add_info=None, help_message=None):
        if _silent_input_enabled():
            # Mirror interactive behavior: print chosen value.
            print(default_value)
    ...
    def input_str(self, s, default_value=None, valid_list=None, show_default_value=True, help_message=None):
        if _silent_input_enabled():
            # Keep existing semantics: when default is None, return None.
