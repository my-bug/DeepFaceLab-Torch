...
class FacesetEnhancerSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            intro_str = '运行设备：%s。' % (client_dict['device_name'])
        ...
        def process_data(self, filepath):
            try:
                ...
                if dflimg is None or not dflimg.has_data():
                    self.log_err (f"{filepath.name} 不是 DFL 图片文件")
                else:
                ...
                self.log_err (f"处理文件时发生异常：{filepath}。错误：{traceback.format_exc()}")
...
def process_folder ( dirpath, cpu_only=False, force_gpu_idxs=None ):
    ...
    io.log_info (f"正在增强 faceset：{dirpath_parts}")
    io.log_info ( f"输出到：{output_dirpath_parts}")
    ...
    is_merge = io.input_bool (f"\r\n将 {output_dirpath_parts} 合并回 {dirpath_parts}？", True)
    if is_merge:
        io.log_info (f"正在复制处理后的文件到 {dirpath_parts}")
        ...
        io.log_info (f"正在移除 {output_dirpath_parts}")
