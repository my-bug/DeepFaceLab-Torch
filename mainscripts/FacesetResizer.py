...
class FacesetResizerSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            self.log_info (f"运行设备：{ client_dict['device_name'] }")
        ...
        def process_data(self, filepath):
            try:
                ...
                if dflimg is None or not dflimg.has_data():
                    ...
                    h,w = img.shape[:2]
                    if h != w:
                        raise Exception(f'宽高不相等（w != h）：{filepath}')
...
def process_folder ( dirpath):
    ...
    image_size = io.input_int(f"新的图片尺寸", 512, valid_range=[128,2048])
    ...
    io.log_info (f"正在调整 faceset 尺寸：{dirpath_parts}")
