...
class AMPModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run():
            resolution = io.input_int("分辨率", default_resolution, add_info="64-640", help_message="更高分辨率需要更多显存和训练时间。该值会自动调整为 32 的倍数。")
            resolution = np.clip ( (resolution // 32) * 32, 64, 640)
            self.options['resolution'] = resolution
            self.options['face_type'] = io.input_str ("人脸类型", default_face_type, ['f','wf','head'], help_message="whole face / head").lower()
        ...
        if self.is_first_run():
            self.options['ae_dims']    = np.clip ( io.input_int("自编码器维度（AE dims）", default_ae_dims, add_info="32-1024", help_message="所有人脸信息会被压缩到 AE dims 中。维度不足可能导致闭眼等细节无法还原。维度越大越好，但更占显存。可用于微调模型大小以适配显卡。" ), 32, 1024 )
            self.options['inter_dims'] = np.clip ( io.input_int("中间层维度（Inter dims）", default_inter_dims, add_info="32-2048", help_message="应大于等于 AE dims。维度越大越好，但更占显存。可用于微调模型大小以适配显卡。" ), 32, 2048 )
            ...
            morph_factor = np.clip ( io.input_number ("形变系数（Morph factor）", default_morph_factor, add_info="0.1 .. 0.5", help_message="常用值为 0.5"), 0.1, 0.5 )
        ...
        if self.is_first_run() or ask_override:
            ...
            self.options['lr_dropout']  = io.input_str (f"使用学习率 dropout", default_lr_dropout, ['n','y','cpu'], help_message="当人脸训练足够后，可启用以获得额外锐度并减少亚像素抖动，从而用更少迭代达到更好效果。建议在“关闭 random warp”之前、以及启用 GAN 之前开启。\n\nn - 关闭\ny - 开启\ncpu - 在 CPU 上启用（节省显存，但单次迭代时间约增加 20%）。")
        ...
        if self.is_first_run() or ask_override:
            ...
            self.options['gan_power'] = np.clip ( io.input_number ("GAN 强度（GAN power）", default_gan_power, add_info="0.0 .. 5.0", help_message="迫使网络学习更细小的人脸细节。仅建议在关闭 random_warp 且人脸已足够清晰时开启，且开启后不要再关闭。值越大伪影概率越高。常用值 0.1。"), 0.0, 5.0 )
            ...
            self.options['ct_mode'] = io.input_str (f"src faceset 的颜色迁移模式", default_ct_mode, ['none','rct','lct','mkl','idt','sot'], help_message="将 src 样本的颜色分布调整得更接近 dst。若 src faceset 足够多样，多数情况下 lct 就很好用。")
        ...
        with tf.device (models_opt_device):
            ...
            if self.is_training:
                ...
                if gan_power != 0:
                    ...
                    self.model_filename_list += [ [self.GAN, 'GAN.pth'],
