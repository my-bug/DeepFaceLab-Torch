...
class SAEHDModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run():
            resolution = io.input_int("分辨率", default_resolution, add_info="64-640", help_message="更高分辨率需要更多显存和训练时间。该值会自动调整为 16 的倍数（以及 -d 架构需要的 32 的倍数）。")
            resolution = np.clip ( (resolution // 16) * 16, min_res, max_res)
            self.options['resolution'] = resolution



            self.options['face_type'] = io.input_str ("人脸类型", default_face_type, ['h','mf','f','wf','head'], help_message="Half / mid face / full face / whole face / head。Half 分辨率更高但覆盖脸颊更少；Mid 比 Half 宽约 30%；Whole face 覆盖含额头的整张脸；Head 覆盖整头，但需要 src/dst faceset 都有 XSeg。").lower()

            while True:
                archi = io.input_str ("AE 架构", default_archi, help_message=\
"""
...
""").lower()
        ...
        if self.is_first_run():
            self.options['ae_dims'] = np.clip ( io.input_int("自编码器维度（AE dims）", default_ae_dims, add_info="32-1024", help_message="所有人脸信息会被压缩到 AE dims 中。如果维度不足，可能出现闭眼等细节无法还原。维度越大越好，但更占显存。可用于微调模型大小以适配显卡。" ), 32, 1024 )

            e_dims = np.clip ( io.input_int("编码器维度（E dims）", default_e_dims, add_info="16-256", help_message="维度越大越容易学习更多面部特征并获得更锐利的结果，但更占显存。可用于微调模型大小以适配显卡。" ), 16, 256 )
            self.options['e_dims'] = e_dims + e_dims % 2

            d_dims = np.clip ( io.input_int("解码器维度（D dims）", default_d_dims, add_info="16-256", help_message="维度越大越容易学习更多面部特征并获得更锐利的结果，但更占显存。可用于微调模型大小以适配显卡。" ), 16, 256 )
            self.options['d_dims'] = d_dims + d_dims % 2

            d_mask_dims = np.clip ( io.input_int("解码器 Mask 维度（D mask dims）", default_d_mask_dims, add_info="16-256", help_message="通常 mask 维度 = 解码器维度 / 3。如果你手工从 dst mask 中抠除遮挡物，可以增大该参数以提升质量。" ), 16, 256 )
            self.options['d_mask_dims'] = d_mask_dims + d_mask_dims % 2

        if self.is_first_run() or ask_override:
            if self.options['face_type'] == 'wf' or self.options['face_type'] == 'head':
                self.options['masked_training']  = io.input_bool ("启用 Masked training", default_masked_training, help_message="仅适用于 'whole_face' 或 'head'。启用后会将训练区域裁剪到 full_face mask 或 XSeg mask，从而更专注训练人脸区域。")

            self.options['eyes_mouth_prio'] = io.input_bool ("眼睛与嘴部优先", default_eyes_mouth_prio, help_message='有助于修复训练中的眼睛问题（如“外星眼”、视线方向错误），同时提升牙齿细节。')
            self.options['uniform_yaw'] = io.input_bool ("样本 yaw 均匀分布", default_uniform_yaw, help_message='当 faceset 侧脸较少导致侧脸模糊时，此选项有帮助。')
            self.options['blur_out_mask'] = io.input_bool ("模糊 Mask 外围", default_blur_out_mask, help_message='对训练样本的人脸 mask 外侧邻近区域进行模糊处理，使换脸后脸部边缘附近背景更平滑、更不显眼。需要 src/dst faceset 都具备准确的 XSeg mask。')
        ...
        if self.is_first_run() or ask_override:
            self.options['models_opt_on_gpu'] = io.input_bool ("将模型与优化器放在 GPU", default_models_opt_on_gpu, help_message="单 GPU 训练时，默认会把模型与优化器权重放在 GPU 上以加速。也可放到 CPU 以释放额外显存，从而使用更大的维度。")

            self.options['adabelief'] = io.input_bool ("使用 AdaBelief 优化器？", default_adabelief, help_message="AdaBelief 更占显存，但通常精度与泛化更好。")

            self.options['lr_dropout']  = io.input_str (f"使用学习率 dropout", default_lr_dropout, ['n','y','cpu'], help_message="当人脸训练足够后，可启用以获得额外锐度并减少亚像素抖动，同时可减少所需迭代数。建议在“关闭 random warp”之前、以及启用 GAN 之前开启。\n\nn - 关闭\ny - 开启\ncpu - 在 CPU 上启用（不占额外显存，但单次迭代时间约增加 20%）。")

            self.options['random_warp'] = io.input_bool ("启用样本随机形变（random warp）", default_random_warp, help_message="随机形变有助于泛化表情。当人脸训练足够后，可关闭以获得额外锐度并减少亚像素抖动，同时可减少所需迭代数。")

            self.options['random_hsv_power'] = np.clip ( io.input_number ("随机色相/饱和度/亮度强度", default_random_hsv_power, add_info="0.0 .. 0.3", help_message="仅对 src faceset 在网络输入处应用随机 HSV 强度，帮助稳定换脸时的颜色扰动。会降低颜色迁移的精确度，因此 src faceset 需要足够多样。常用值 0.05。"), 0.0, 0.3 )

            self.options['gan_power'] = np.clip ( io.input_number ("GAN 强度（GAN power）", default_gan_power, add_info="0.0 .. 5.0", help_message="迫使网络学习更细小的人脸细节。仅建议在启用 lr_dropout 且关闭 random_warp、并且人脸已足够清晰时开启，且开启后不要再关闭。值越大伪影概率越高。常用值 0.1。"), 0.0, 5.0 )

            if self.options['gan_power'] != 0.0:
                gan_patch_size = np.clip ( io.input_int("GAN patch 大小", default_gan_patch_size, add_info="3-640", help_message="patch 越大质量越高，但更占显存。即使最低设置也能获得更锐利的边缘。常用值为 分辨率/8。" ), 3, 640 )
                self.options['gan_patch_size'] = gan_patch_size

                gan_dims = np.clip ( io.input_int("GAN 维度（GAN dims）", default_gan_dims, add_info="4-512", help_message="GAN 网络维度。维度越大越占显存。即使最低设置也能获得更锐利的边缘。常用值 16。" ), 4, 512 )
                self.options['gan_dims'] = gan_dims

            if 'df' in self.options['archi']:
                self.options['true_face_power'] = np.clip ( io.input_number ("'True face' 强度", default_true_face_power, add_info="0.0000 .. 1.0", help_message="实验性选项：让结果更像 src。值越大约束越强。常用值 0.01。对比图：https://i.imgur.com/czScS9q.png"), 0.0, 1.0 )
            else:
                self.options['true_face_power'] = 0.0

            self.options['face_style_power'] = np.clip ( io.input_number("人脸风格强度（face style）", default_face_style_power, add_info="0.0..100.0", help_message="学习让预测人脸在 mask 内部的颜色更接近 dst。若与 'whole_face' 搭配，需要使用训练好的 XSeg mask。警告：建议在 10k+ 迭代、预测人脸足够清晰后再开启；从 0.001 起逐步增加并观察历史变化。开启会增加模型崩溃概率。"), 0.0, 100.0 )
            self.options['bg_style_power'] = np.clip ( io.input_number("背景风格强度（bg style）", default_bg_style_power, add_info="0.0..100.0", help_message="学习让预测人脸 mask 外部区域更接近 dst。若与 'whole_face' 搭配，需要使用训练好的 XSeg mask。开启会增加模型崩溃概率。常用值 2.0。"), 0.0, 100.0 )
    ...
    def on_initialize(self):
        ...
        with tf.device (models_opt_device):
            if 'df' in archi_type:
                ...
                self.model_filename_list += [ [self.encoder,     'encoder.pth'    ],
